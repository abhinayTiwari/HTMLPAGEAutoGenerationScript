...
const template = fs
  .readFileSync(RISEModel.inputPaths.programTemplatePath)
  .toString(); //needs to change
const speaker_template = fs
  .readFileSync(RISEModel.inputPaths.speakerTemplate)
  .toString(); // needs to change
const rise_speaker_template = fs
  .readFileSync(RISEModel.inputPaths.riseAllSpeakers)
  .toString(); //needs to change
...
function Panelist(firstName, lastName, link, bio, title, institution, role) {
  ...
  this.institution = (institution == null || institution == "0") ? "" : institution;
  this.role = role;
}
...
function sortPanelistsByLastName(Sessions) {
  for (sessionCount = 0; sessionCount < Sessions.length; sessionCount++) {
    for (var i = 0; i < Sessions[sessionCount].Panelists.length; i++) {
      for (
        var j = 0;
        j < Sessions[sessionCount].Panelists.length - (i + 1);
        j++
      ) {
...
function createAgendaPages(Sessions) {
  for (let day = 1; day <= 3; day++) {
    ...
    fs.writeFileSync(
      targetlocationFolder.concat("agenda-day") + day + ".html",
      agenda_template
    );
  }
}
