...
const excelfilepath = 'C:\\Users\\UM127517\\Downloads\\Participating_Institutions.xlsx';

readXlsxFile(excelfilepath, {sheet: 'Sheet1'}).then((rows)=>{
    const template = fs.readFileSync('C:\\xampp\\htdocs\\rise2019dev\\participating-institutions-template.html').toString();
    let content = buildContent(rows);
    content = template.replace('{Content}', content);
    fs.writeFileSync('C:\\xampp\\htdocs\\rise2019dev\\participatinginstitutions.html', content);

})

function buildContent(rows)
{
    const institution_types = ['University/College','Government','Media','Nonprofit','Industry/Business', 'Research Center', 'Task Force'];
    const institutionIds = ['higher-Ed', 'government', 'media', 'nonprofit', 'industrybusiness', 'researchcenter', 'task-force'];
    let content = '';

    for(let institution_type_count = 0; institution_type_count < institution_types.length; institution_type_count++ )
    {
        let filtered_rows = rows.filter(row => row[1] == institution_types[institution_type_count]);
        let institutions = filtered_rows.map(function(value, index) {return value[0]});//get columns
        ...
        for(let rowcount = 0; rowcount < institutions.length; rowcount++)
        {
            ...
            institutionTypeContent= institutionTypeContent.concat(institutionRowElem);
        }
        let institutionContent = buildInstitutionTemplate(institutionIds[institution_type_count], institution_types[institution_type_count], institutionTypeContent);
...
function buildInstitutionTemplate(institutionId, institutionType, institutionTypeContent)
{
    let institutionContent = 
        `<div class="field--item" id= "${institutionId}">
